...
struct tx_op final : iox::op_base {
    ...
    tx_op(socket* s, frame frame, R&& recv) noexcept
        : op_base(&tx_op::on_done), xsk(s), f(frame), r(std::move(recv)) {}
...
struct tx_sender {
    ...
    using completion_signatures =
        stdexec::completion_signatures<stdexec::set_value_t(std::uint32_t)>;
    template <class Self, class R>
    auto connect(this Self&& self, R&& r) {
        return detail::tx_op<std::remove_cvref_t<R>>{self.xsk, self.f, std::forward<R>(r)};
    }
};
...
struct write_frame_t {
    ...
    auto operator()(io_context&, S& s, frame f) const noexcept {
        return tx_sender{&s, f};
    }
};
